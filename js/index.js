...
(async function () {
  const baseBox = document.querySelector('.header-box .base'),
    innerBox = document.querySelector('.inner'),
    wrapperBox = document.querySelector('.wrapper1'),
    wrapper = document.querySelector('.cover .wrapper'),
  ...
  let wrapperList = [], timer = null, matchNum = 0
console.log(innerBox);
  ...
  const handle = function handle() {
    ...
    let { minutes: currentTimeMinutes, seconds: currentTimeSeconds } = format(currentTime), /*  加： 是重命名 */
  ...
  const binding = function binding(data) {
    let { title, author, duration, pic1, pic2, pic3, audio, lyric } = data;

    //@2 杂七杂八的信息
    durationBox.innerHTML = duration
    markImageBox.style.backgroundImage = `url(${pic3})`;
    audioBox.src = audio
